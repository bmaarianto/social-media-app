...
const ChatBox = () => {
  ...
  return user && (
    <div className="flex flex-col h-screen">
              ...
              <div key={index} className={`flex flex-col ${message.to_user_id !== user._id ? 'items-start' : 'items-end'}`}>
                <div className={`p-2 text-sm max-w-sm bg-white text-slate-700 rounded-lg shadow 
                  ...
                  {message.message_type === 'image' && (
                    <img src={message.media_url} alt="Media" className="w-full max-w-sm rounded-lg mb-1" />
                  )}
                  <p>{message.text}</p>
                </div>
              </div>
      ...
      <div className="px-4">
        <div className="flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow 
        rounded-full mb-5">
